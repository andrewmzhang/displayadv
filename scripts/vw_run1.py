...
class Option(Enum):
    ...
    def features(cls, val):
        if val == Option.quad_11:
            ...
            return ' '.join(quadratic(list('pnabcdg'), excd=['aa', 'bb', 'cc', 'gg'])) \
                + ' --save_resume --feature_mask log_bin1.model --l2 10e-8 --l1 0e-8  -l 0.005 -q m:'
        elif val == Option.quad_13:
            return ' '.join(quadratic(list('pnabcdg'), excd=['aa', 'bb', 'cc', 'gg'])) \
                + ' --save_resume --feature_mask log_bin1.model --l2 10e-8 --l1 1.3e-8 -l 0.007 -q m:'
